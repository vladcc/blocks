...
{	
	...
	/*** test trivial case ***/
	{
		...
		check(pio.match_block_name(r_name));
		...
		// match {
		check(pio.line_pos() == 0);
		check(pio.match_first_of(r_open, r_close) == 1);
		check(pio.line_num() == 1);
		check(pio.line_pos() == 5);
		
		// match }
		check(pio.line_pos() == 5);
		check(pio.match_first_of(r_open, r_close) == 2);
		check(pio.line_num() == 1);
		...
		check(pio.line_num() == 1);
		check(!pio.match_block_name(r_name));
		check(!pio.match_first_of(r_open, r_close));
	...
	/*** test multi line case ***/
	{
		...
		check(pio.line_pos() == 6);
		check(pio.match_first_of(r_open, r_close) == 0);
		check(pio.read_line());
...
static bool test_block_parser()
{
	...
	block_parser::regexps expressions(&r_name, &r_open, &r_close);
